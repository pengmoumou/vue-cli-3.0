...
function Snake() {
    ...
    //画出4个方块，设置成灰色
    for (var i = 0; i < 4; i++) {
        var rect = new Rect(i * 20, 0, 20, 20, "#f9e0ce");
...
Snake.prototype.move = function(canvas,timer) {
    ...
    //3、砍去末尾的方块
    //4、将蛇头向设定方向移动一格
    var rect = new Rect(this.head.x, this.head.y, this.head.w, this.head.h, "#f9e0ce");
...
function getRandomFood(canvas,snake) {
    ...
    while (isOnSnake) {
        ...
        var rect = new Rect(indexX * 20, indexY * 20, 20, 20, "#492002");
