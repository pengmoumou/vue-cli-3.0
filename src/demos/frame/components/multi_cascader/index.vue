<template>
  <span
    class="el-cascader multi-cascader"
    :class="[
      {
  ...
  methods: {
    ...
    handleKeydown(e) {
      ...
      if (keyCode === 13) {
        ...
        setTimeout(() => {
          const firstMenu = this.popperElm.querySelectorAll(
            ".el-cascader-menu"
          )[0];
    ...
    handlePick(value, item, close = true) {
      // console.log(value);
      this.currentValue = value;
    ...
    handleInputChange(value) {
      ...
      let filteredFlatOptions = flatOptions.filter(optionsStack => {
        return optionsStack.some(option =>
          new RegExp(escapeRegexpString(value), "i").test(option[this.labelKey])
        );
      });
