...
export default {
  ...
  methods: {
    select(item, menuIndex) {
      ...
      this.handleMultiValue(item);
      // console.log(this.multiValue);
      this.$emit("pick", this.multiValue, this.multiItem);
      // this.$emit("pick", this.activeValue.slice());
    },
    handleMultiValue(item) {
      let length = this.multiValue.length;
      if (length > 0) {
        this.multiValue.forEach((itemVaule, index) => {
          // console.log(index);
      ...
      if (this.multiValue.length == length) {
  ...
  render(h) {
    ...
    const menus = this._l(activeOptions, (menu, menuIndex) => {
      ...
      const items = this._l(menu, item => {
        ...
        let handleMultiActive = function(item) {
          let isActive = false;
          multiValue.forEach(itemVaule => {
    ...
    return (
      ...
      >
        <div
          v-show={visible}
          class={["el-cascader-menus el-popper multi-menus", popperClass]}
          ref="wrapper"
        >
...
<style lang="scss">
.multi-menus {
  .el-cascader-menu__item.is-active::after {
    position: absolute;
    right: 20px;
